...
export async function collectMarketViewInput({ outputDir = join("runs", "market-view", "live-source-cache") } = {}) {
  ...
  const input = {
    ...
    traders: [
      ...
      {
        name: "BigTrout300",
        display_name: "BigTrout300",
        source: "Hypurrscan",
        account_stats: "Live scrape maxAge=0",
        status: parseHypurrscanTrader(bigTrout300.markdown).length ? undefined : "no_active_positions",
        positions: parseHypurrscanTrader(bigTrout300.markdown),
      },
      {
        name: "DegenDuck",
